...
async def test_full_session_flow():
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=30) as client:
        ...
        print(
            f"  Mean grounding accuracy:     {analytics['mean_grounding_accuracy_grounded']}"
        )
        print(
            f"  Mean hallucination (grounded):{analytics['mean_hallucination_rate_grounded']}"
        )
        print(
            f"  Mean improvement:            {analytics['mean_grounding_improvement']}"
        )
        print(">>> PASSED")
